...
export function ItemTable({ items }) {
  const columns = useMemo(() => [
    ...
    ...TRADERS.map((trader) =>
      col.accessor((row) => row.trader_prices?.[trader], {
        id: `trader_${trader}`,
        header: trader,
    ...
    col.accessor('difference_pct', {
      ...
      cell: (info) => {
        ...
        const fire = v >= 20 ? ' 🔥' : '';
        return <span className={`font-bold ${color}`}>{v > 0 ? '+' : ''}{v}%{fire}</span>;
      },
    }),
    col.accessor('recommendation', {
      ...
      cell: (info) => {
        const v = info.getValue();
        const labels = {
          BUY_FROM_TRADER: { text: 'Buy from trader', cls: 'bg-green-900 text-green-200' },
        ...
        const label = labels[v] || { text: v, cls: 'bg-gray-700' };
